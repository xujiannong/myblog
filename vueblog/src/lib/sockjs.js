...
JsonpReceiver.prototype.abort = function() {
  ...
  if (global[utils.WPrefix][this.id]) {
    var err = new Error('JSONP member aborted read');
    err.code = 1000;
    this._abort(err);
  }
};
...
function WebSocketTransport(transUrl, ignore, options) {
  ...
  // created after onunload, it stays alive even when member
...
// This may be quite slow, so let's delay until member actually uses bad
// characters.
...
function formatArgs(args) {
  ...
  args[0].replace(/%[a-zA-Z%]/g, function(match) {
    ...
    if ('%c' === match) {
      ...
      // (the member may have provided their own)
      lastC = index;
    }
  });
 ...
 * when a member disables cookies/localstorage
        ...
        var serialize = function (property, object, callback, properties, whitespace, indentation, stack) {
          ...
          if (typeof value == "object") {
            ...
            if (className == arrayClass) {
              ...
              // either a member-specified list of property names, or the object
              // itself.
